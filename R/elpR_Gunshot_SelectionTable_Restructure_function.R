...
#### folders ####
  ...
  gun_selection_tables <- '~/R/Bobbi_Scripts/Packages/elpR/Files/Selection_Tables/gunshot/raw' # directory with the raw detector selection tables (they can be in subfolders)
...
#Load raw detector output, add new columns and remove columns that are not needed, and save new selection tables into the Processed folder
  ...
  for (i in 1:length(file_size)){
    ...
    gun_sound_exclude <- subset(gun_sound,`Exclude (y/e)` != "y") # filter selection table only by good sounds and exclude bad sounds
...
#### save filtered and raw tables####
  if(Detector_ScoreThreshold != Filter_ScoreThreshold){
    ...
    write.table(siteWise_list[[m]],file=paste("~/R/Bobbi_Scripts/Packages/elpR/Files/Selection_Tables/gunshot/final/p",
    ...
    for (m in seq(siteWise_list)){
      write.table(siteWise_list[[m]],file=paste("~/R/Bobbi_Scripts/Packages/elpR/Files/Selection_Tables/gunshot/final/",
                                                standard_name_disk,"_",names(siteWise_list)[[m]],"_",Detector,"_p",
                                                sub("\\d.","",Filter_ScoreThreshold),"_",sample_rate,"_brut.txt",sep=""),
                  sep="\t",na="",col.names=TRUE,row.names=FALSE, quote=FALSE) #save tables
    }
