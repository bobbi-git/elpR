...
##### Sound Check (list of all sound files) #####
  sound_check <- list.files(path = "L://ELP//Projects//Nouabale//nn_analyses//base tables//sound_checks//",recursive=T,full.names=TRUE, pattern = "*.txt") # list all the selection tables in the ele_tables folder
  sound_check_list <- lapply(sound_check, function(x) {read.table(file = x, header = T, sep ="\t", check.names=FALSE,quote = "\"",comment.char = "")})  # Read the files in, assuming tab separator
  ...
  sound_check_merge_df2 <- sound_check_merge_df[,c("Site", "File Name","File Duration (s)","File Start DateTime","File Start Date","File End DateTime","File End Date","Sound Check Count","Exclude (y)","Notes","Deployment Notes","Sound Data Notes","Deployment Number")] # create new dataframe with specific columns
...
##### Hori-Harm Selection Tables (elephant detections (0+)) #####
  ele_tables<-list.files(path = "L://ELP//Projects//Nouabale//nn_analyses//base tables//ele//",recursive=T,full.names=TRUE, pattern = "*.txt") # list all the selection tables in the ele_tables folder
  ele_tables_list <- lapply(ele_tables[c(1:6,8)], function(x) {read.table(file = x, header = T, sep ="\t", check.names=FALSE,quote = "\"",comment.char = "")})  # Read the files in, assuming tab separator (except the one for dep1-3 since we load in one for deps 1-8)
  ...
  #View(ele_dets_df)
  dim(ele_dets_df) #1223034      13
  min(ele_dets_df$`File Start DateTime`) # 2017-12-15 WAT
  max(ele_dets_df$`File Start DateTime`) # 2023-03-04 WAT
  sum(ele_dets_df$`Rumble Count`, na.rm = TRUE) # 57339 rumbles


##### Zero-Days Tables (sound files with 0 hori-harm detections) #####
  zero_txt <- "L://ELP//Projects//Nouabale//nn_analyses//base tables//HH_zero_detection_days//"
...
#create table for every hour from the first to last in detector files
  ...
  sites <- rename(sites,"Site" = "sort(unique(sound_check_merge_df2$Site))")
  nrow(sites) #54 (includes cluster sites)

  smpl_hrs2 <- do.call("rbind", replicate(nrow(sites), smpl_hrs, simplify = FALSE)) # repeat dateTime 54 times for the sites
  nrow(smpl_hrs2) #2468934

  sites2 <- do.call("rbind", replicate(nrow(smpl_hrs), sites, simplify = FALSE)) # repeat the site list n (45721 = number of hours for survey) times
...
#combine sites and hourly times
  ...
  dim(smpl_hrs_site) #2468934 x 4
  max(smpl_hrs_site$DateTime) # "2023-03-04 WAT"
  min(smpl_hrs_site$DateTime) # "2017-12-15 WAT"
...
# combine with rand dates
  ...
  dim(smpl_hrs_site_rand) #2468934 x 5
...
# combing elephant rumbles and zero days
  ...
  dim(ele_dets_df_zero) #  1235691      14
  sum(ele_dets_df_zero$`Rumble Count`) # 57339
  #names(ele_dets_df_zero)
  min(ele_dets_df_zero$Date) # "2017-12-15"
  max(ele_dets_df_zero$Date) # "2023-03-04"
...
# summarize hourly sound check bad data
  soundCheck_exclude_hr <-  sound_check_merge_df2 %>%
            group_by(Date, Hour, Site, `Exclude (y)`) %>%
            filter(`Exclude (y)` != "") %>% # remove files to be excluded
...
# merge with bad gain times
  ...
  setwd('L:/ELP/Projects/Nouabale/nn_analyses/base tables/merged_data')
  write.table(ele_hrly,"PNNN_Rumbles_ZeroDays_badSounds_randNonRand_hourly.txt",sep='\t',na="",col.names=TRUE,row.names=FALSE, quote=FALSE, append=FALSE)

# plot total rumbles and total bad sounds per hour
...
# Note: Deployments 1-3 are excluded because they did not have 3 random days per week
  ele_hrly_rand_excludedSound <-  ele_hrly %>%
    filter(is.na(`Exclude (y)`)) %>% # remove files to be excluded
  ...
  sum(ele_hrly_rand_excludedSound$`Sum Rumbles`,na.rm = TRUE) #36656
  sum(ele_hrly_rand_excludedSound$`Sum Bad Sounds (s)`,na.rm = TRUE) #0
  dim(ele_hrly_rand_excludedSound) #991538      8

  write.table(ele_hrly_rand_excludedSound,"PNNN_Rumbles_ZeroDays_soundExclude_rand_hourly.txt",sep='\t',na="",col.names=TRUE,row.names=FALSE, quote=FALSE, append=FALSE)
