...
# - this script will merge all selections into one table and will rename the Selection ID so they are all unique. It will preserve the original Selection ID in a new column.
...
# For files in folder and subfolders: DOES NOT WORK (merges all tables into one instead of one per folder)!
...
# Begin Clock Time is "NULL" in merged table - need to fix this.
