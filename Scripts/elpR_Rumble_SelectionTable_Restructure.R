...
rm(list = ls())
  ...
  deployment_name <- "gr_202405_may" # official name of the deployment
  deployment_num <- "01" # update with current deployment number
  disk_ID <- "GridA" # name of the disk where the sound files are stored (alphanumeric)
  sites <- "GridA_Sites.txt" # Name of the text file in the 'sites' folder that has a list of the sites for this project, include file type extension in name (e.g., 'txt)
